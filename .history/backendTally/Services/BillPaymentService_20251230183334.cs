...
namespace backendTally.Services
{
    public class BillPaymentService 
    {
        ...
        public async Task<BillPayment> MarkBillAsPaid(int userId, int recurringBillId, DateTime paidDate)
        {
            ...
            _context.BillPayments.Add(pay)

            return payment;
        }

        public async Task<bool> UnmarkBillAsPaid(int userId, int recurringBillId, DateTime paidDate)
        {
            var payment = await _context.BillPayments
                .Where(p => p.UserId == userId && p.RecurringBillId == recurringBillId && p.PaidDate == paidDate)
                .FirstOrDefaultAsync();
