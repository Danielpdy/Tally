...
namespace backendTally.Services
{
    public class AggregateService
    {
        ...
        {
            var endDate = DateOnly.FromDateTime(DateTime)
            ...
            for (int i = 0; i < 7; i++)
            {
                ...
                result.Add(new
                {
                    name = currentDate.DayOfWeek.ToString().Substring(0, 3),

                    date = currentDate.ToString("yyyy-MM-dd"),

                    Income = dayAggregate?.TotalEarnings ?? 0,
                    Expenses = dayAggregate?.TotalSpendings ?? 0,
                    net = dayAggregate?.NetAmount ?? 0
                });
            }
