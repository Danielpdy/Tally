...
namespace backendTally.Controllers
{
    ...
    public class BillPaymentController : ControllerBase
    {
        ...
        public async Task<ActionResult<BillPayment>> AddNewBillPayment([FromBody] NewBillPaymentDto dto)
        {
            ...
            try
            {
                var payment = await _billPaymentService.MarkBillAsPaid(authenticatedUserId, dto.RecurringBillIdecurringBillId, dto.PaidDate);
        ...
        public async Task<IActionResult> DeleteBillPaymentById(int recurringBillId, [FromQuery] DateTime paidDate)
        {
            var userIdClaim = User.FindFirst(ClaimTypes.NameIdentifier)?.Value;
            ...
            await _billPaymentService.UnmarkBillAsPaid(authenticatedUserId, recurringBillId, paidDate);
