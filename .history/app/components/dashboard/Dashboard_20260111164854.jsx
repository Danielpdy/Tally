...
const Dashboard = () => {
    ...
    const monthlyIncome = 5240;
    const monthlyExpenses = 3890;
    const monthlyChange = 1350;
    const safeToSpend = 1350;
    const budgetPerformance = "+18% under your average";
    ...
    const spendingCategories = [
        { name: 'Entertainment', percentage: 25, color: '#8B5CF6', emoji: '🎮' },
        { name: 'Food', percentage: 22, color: '#F59E0B', emoji: '🍔' },
        { name: 'Other', percentage: 15, color: '#94A3B8', emoji: '🛒' },
        { name: 'Rent', percentage: 18, color: '#8B5CF6', emoji: '🏠' },
        { name: 'Subscriptions', percentage: 10, color: '#38BDF8', emoji: '📱' },
        { name: 'Transport', percentage: 10, color: '#10B981', emoji: '🚗' }
    ];
            ...
            <div className={styles.dashboardPage}>
                ...
                <div className={styles.topStatsRow}>
                    <div className={styles.statCard}>
                        ...
                        <h2 className={styles.statAmount}>${totalBalance.toLocaleString()}</h2>
                    ...
                    <div className={styles.statCard}>
                        ...
                        <div className={styles.flowItems}>
                            <div className={styles.flowItem}>
                                ...
                                <span className={styles.flowAmount}>${monthlyIncome.toLocaleString()}</span>
                                <span className={styles.flowLabel}>Income</span>
                            </div>
                            <div className={styles.flowItem}>
                                <span className={styles.flowIcon} style={{ color: '#FF8042' }}>↙</span>
                                <span className={styles.flowAmount} style={{ color: '#FF8042' }}>${monthlyExpenses.toLocaleString()}</span>
                                <span className={styles.flowLabel}>Expenses</span>
                            </div>
                        </div>
                        <p className={styles.monthlyChange}>+${monthlyChange} this month</p>
                    </div>
                ...
                <div className={styles.mainGrid}>
                    ...
                    <div className={styles.leftColumn}>
                        ...
                        <div className={styles.card}>
                            <h3 className={styles.cardTitle}>Where Your Money Went</h3>
                            <div className={styles.pieChartContainer}>
                                <div className={styles.donutChart}>
                                    <svg viewBox="0 0 200 200" className={styles.donutSvg}>
                                        <circle cx="100" cy="100" r="80" fill="none" stroke="#8B5CF6" strokeWidth="40" strokeDasharray="126 377" transform="rotate(-90 100 100)" />
                                        <circle cx="100" cy="100" r="80" fill="none" stroke="#F59E0B" strokeWidth="40" strokeDasharray="110 503" strokeDashoffset="-126" transform="rotate(-90 100 100)" />
                                        <circle cx="100" cy="100" r="80" fill="none" stroke="#94A3B8" strokeWidth="40" strokeDasharray="75 628" strokeDashoffset="-236" transform="rotate(-90 100 100)" />
                                        <circle cx="100" cy="100" r="80" fill="none" stroke="#A78BFA" strokeWidth="40" strokeDasharray="90 703" strokeDashoffset="-311" transform="rotate(-90 100 100)" />
                                        <circle cx="100" cy="100" r="80" fill="none" stroke="#38BDF8" strokeWidth="40" strokeDasharray="50 793" strokeDashoffset="-401" transform="rotate(-90 100 100)" />
                                        <circle cx="100" cy="100" r="80" fill="none" stroke="#10B981" strokeWidth="40" strokeDasharray="52 843" strokeDashoffset="-451" transform="rotate(-90 100 100)" />
                                    </svg>
                                </div>
                                <div className={styles.categoryLegend}>
                                    {spendingCategories.map((cat, idx) => (
                                        <div key={idx} className={styles.legendItem}>
                                            <div className={styles.legendColor} style={{ backgroundColor: cat.color }}></div>
                                            <span className={styles.legendText}>{cat.name} {cat.emoji}</span>
                                        </div>
                                    ))}
                                </div>
                            </div>
                    ...
                    <div className={styles.rightColumn}>
                        ...
                        <div className={styles.card}>
                            ...
                            <p className={styles.safeToSpendSubtext}>{budgetPerformance}</p>
                        </div>
