...
const Transactions = () => {
    ...
    const [addCategory, setAddCategory] = use
    ...
    if ( transactionsNumber <= 0 ) {
        return(
            ...
            <aside ref={sidePannelRef} className={`${styles.aside} ${sidePannelToogle ? styles.visible : ""}`}>
                ...
                <div className={`${currentStepPannel === steps.SELECT_CATEGORY ? "" : styles.hidden}`}>
                    <p>Select Category</p>
                    <p>Choose the specific category for this income.</p>
                    <div>
                        <label htmlFor="Category">Category</label>
                        <div>
                            <button onClick={handleToggleDropdown}>Choose a category</button>
                            {isOpen && (
                                <div>
                                    <div>
                                        {categories.map((category, index) => (
                                            <div key={index} onClick={handleCloseDropdown}>
                                                {category}
                                            </div>
                                        ))}
                                    </div>
