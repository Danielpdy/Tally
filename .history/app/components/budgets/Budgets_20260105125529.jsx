...
const Budgets = () => {
  ...
  const fetchBudgetGoal = async () => {
    ...
    try {
      ...
      if (!data || !data.budgetGoal) return;

      setBudgetGoal(data.budgetGoal.targetAmount);
      setDailyLimit(data.daysLeft > 0 ?(data.budgetGoal.targetAmount / data.daysLeft).toFixed(2) : 0);
  ...
  const handleAddBudgetGoal = async (e) => {
    ...
    try {
      ...
      setDaysLeft(data.daysLeft);
      setDailyLimit(data.daysLeft > 0 ?(budgetAmount / data.daysLeft).toFixed(2) : 0);
      setBudgetAmount(0);
  ...
  const updateBudgetGoal = async (e) => {
    ...
    try{
      ...
      setBudgetGoal(data.targetAmount);
      setDaysLeft(data.daysLeft)
      setDailyLimit(data.daysLeft > 0(budgetAmount / data.daysLeft).toFixed(2));
      setBudgetAmount(0);
      setIsCustomBudget(false);
    } catch (error){
      console.error(error);
