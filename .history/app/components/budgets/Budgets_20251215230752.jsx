...
const Budgets = () => {
  ...
  const calculateSafeToSpend = () => {
    const percentage = bufferPercent / 100;

    const totalIncome = weeklySpendings + calculateTotalBills;

    const amount = percentage * totalIncome;
    console.log(`Testing: ${weeklyEarnings}`);
    setBufferAmount(percentage * totalIncome);

    const saveToSpendAmount = weeklyEarnings - weeklySpendings - calculateTotalBills - bufferAmount;
  }
                ...
                {!isLoadingSafe && !isErrorSafe && (
                  <>
                    ...
                    <div className={`${styles.breakdownItem} ${styles.buffer}`}>
                      ...
                      <p className={styles.breakdownAmount}>${bufferAmount}</p>
                    </div>
                  </>
                )}
              </div>
