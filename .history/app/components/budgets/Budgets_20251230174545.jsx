...
import { GetPaidBills, MarkBillAsPaid } from "@services/BillPaymentService";
...
const Budgets = () => {
  ...
  const fetchBillPayments = async () => {
    try{
      data = await GetPaidBills(session.accessToken);
  ...
  const handleAddBillPayment = async (recurringBillId) => {
    ...
    console.log(recurringBillId);

    try{
      const data = await MarkBillAsPaid(recurringBillId, payDate, session.accessToken);

      setRecurringBills((prevBills) => 
        prevBills.map((bill) => 
        bill.id === recurringBillId ? { ...bill, status: "Paid this week"}
        : bill
      )
    );

    setBillsPaidThisWeekIds((prev) => [...prev, data.payment.recurringBillId]);
  ...
  }, [recurringBills, billsDueThisWeekIds, billsOverdueThisWeekIds])
