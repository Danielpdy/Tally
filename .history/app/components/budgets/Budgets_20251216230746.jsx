...
const Budgets = () => {
  ...
  console.log('Due IDs:', billsDueThisWeekIds);
  console.log('Bill IDs:', recurringBills.map(b => b.id));
  
  const updatedBills = recurringBills.map(bill => {
    const isMatch = billsDueThisWeekIds.includes(bill.id);
    console.log(`Bill ${bill.id} match check:`, isMatch); // ← Log each check
    return {
      ...bill,
      status: isMatch ? 'Due this week' : undefined
    };
  });
