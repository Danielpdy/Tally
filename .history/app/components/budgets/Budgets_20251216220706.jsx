...
const Budgets = () => {
  ...
  useEffect(() => {
  // When recurring bills are loaded, mark which ones are due
  if (recurringBills.length > 0 && totalBillsDue !== undefined) {
    fetchBillsDueThisWeek(); // This will now have bills to update
  }
}, [recurringBills.length]); // Trigger when bills load
  ...
  const fetchBillsDueThisWeek = async () => {
    try{
      ...
      if(recurringBills.length > 0){
        const dueThisWeekIds = data.upcomingBills.map(b => b.id);
        const updatedBills = recurringBills.map(b => ({
          ...b,
          status: dueThisWeekIds.includes(b.id) ? 'Due this week' : undefined
        }));
