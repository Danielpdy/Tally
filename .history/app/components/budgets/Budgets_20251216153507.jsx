...
const Budgets = () => {
  ...
  const [safetyBuffer, setSafetyBuffer] = useState(() => calculateSafeToSpend());
  ...
  const calculateSafeToSpend = useMemo(() => {
    if (!bufferPercent) {
      setSafetyBuffer("A buffer percentage must be selected.")
    }

    setSafetyBuffer((bufferPercent / 100) * earnings);
  }, [bufferPercent]);
