...
const Budgets = () => {
  ...
  const handleAddBill = async (e) => {
    ...
    const billData = {
      ...
      dayOfMonth: billForm.dayOfMonth,
    ...
    if (billData.dayOfMonth < 1 || billData.dayOfMonth > 31){
      alert("Day must be be between 1 and 31");
      return;
    }

    setRecurringBills(prev => [...(prev || []), {...billData}]);
    ...
    try {
      ...
      const result = await AddRecurringBill(billForm, session.accessToken);
      setRecurringBills(prev => prev.map(b => b.id === tempId ? result : b));
      setBillAdded(true);
    } catch (error) {
      console.error("Failed to add bill:", error);
      setRecurringBills(prev => prev.filter(b => b.id !== tempId));
      setIsErrorBill("Failed to add bill. Please try again");
    }
    finally{
      setIsLoadingBill(false);
        ...
        <div className={styles.twoColumnLayout}>
          ...
          <div className={styles.rightColumn}>
            ...
            <div className={styles.recurringBillsCard}>
              ...
              {isErrorBill && (
                <div>
                  <p>An error ocurrend when loading your bills. Please try again</p>
                  <button
              ...
              {recurringBills && !isLoadingBill && !isErrorBill && (
                <>
