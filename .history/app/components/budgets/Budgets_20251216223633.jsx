...
const Budgets = () => {
  ...
  const [billsDueThisWeekIds, setBillsDueThisWeek] = useState([]);
  ...
  useEffect(() => {
    if(recurringBills.length > 0 && billsDueThisWeek.length > 0){
      const updatedBills = recurringBills.map(bill => ({
        ...bill,
        status: billsDueThisWeek.includes(bill.id) ? 'Due this week' : undefined
      }));
