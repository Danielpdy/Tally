...
const SpendingChart = ({ preview, content = [], recurringBillsDueThisWeek = [], recurringBillsOverdueThisWeek = []}) => {
    ...
    const currentWeekData = useMemo(() => {
      ...
      const unpaidBillsAmount = () => {

        const billsDueThisWeek = recurringBillsDueThisWeek.reduce((sum, bill) =>{
            sum += bill.amount
            return sum;
        }, 0);

        const billsOverdueThisWeek = recurringBillsOverdueThisWeek.reduce((sum, bill) => {
            sum += bill.amount
            return sum;
        }, 0);

        return billsDueThisWeek + billsOverdueThisWeek;
    }



      /*// Find unpaid recurring bills due this week
      const unpaidBillsThisWeek = recurringBills.filter((bill) => {
        // Check if bill has a dueDate
        if (!bill.dueDate) {
          return false;
        }
        ...
        const billDueDate = new Date(year, month - 1, day);

        if (billDueDate < weekStart || billDueDate > weekEnd) {
          return false;
        }

        // Check if bill has been paid
        const isPaid = paidBills.some((paidBill) => paidBill.recurringBillId === bill.id);
        return !isPaid;
      });

      // Convert unpaid bills to transaction format
      const billTransactions = unpaidBillsThisWeek.map((bill) => ({
        type: "Expense",
        amount: bill.amount,
      ...
      return [...weekTransactions, ...billTransactions];
      */
    }, [content, recurringBills, paidBills]);
